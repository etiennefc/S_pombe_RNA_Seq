...
conditions = c("H2O2_ip", "Stat_ip", "Heat_ip", "Normal_ip") # We are interested here in the stress_ip vs normal_ip and also stress ip vs other stress ip (fold change with regards to normal in the first comparison)
for (i in 1:3) {
	for (j in (i+1):4) {
		cnd1 = conditions[i]
		cnd2 = conditions[j]
